...
export default function AbbosRefusedScreen() {
  const { t } = useTranslation();
  const navigation = useNavigation();
  const router = useRouter();
  const { refusal, swipeRight, removeCard } = useSwipe();
  ...
  const filteredRefusal = React.useMemo(() => {
    let result = cards.filter((card) => {
      let matches = refusal.some((item) => item.id === card.id);
  ...
  const AbbosHandleRemove = (cardId: string) => {
    removeCard(cardId, 'refusal');
  };

  const AbbosHandleMoveToChosen = (card: CardData) => {
    setSelectedCard(card);
    setModalVisible(true);
  };

  const AbbosConfirmMoveToChosen = () => {
    if (selectedCard) {
      removeCard(selectedCard.id, 'refusal');
      swipeRight(selectedCard);
    }
  ...
  return (
    <View style={AbbosStyles.container}>
      ...
      <FlatList
            ...
            <View style={AbbosStyles.cardContent}>
              ...
              <View style={AbbosStyles.actionContainer}>
                <TouchableOpacity
                  ...
                  onPress={() => AbbosHandleMoveToChosen(item)}
                >
        ...
        ListEmptyComponent={<Text style={AbbosStyles.emptyText}>{t('no_refused')}</Text>}
      />
        ...
        <View style={AbbosStyles.modalOverlay}>
          <View style={AbbosStyles.modalContentBox}>
            ...
            <Text style={AbbosStyles.modalText}>
              {selectedCard?.title} {t('add_to_chosen')}
            </Text>
            <TouchableOpacity
              style={AbbosStyles.modalButton}
              onPress={AbbosConfirmMoveToChosen}
            >
